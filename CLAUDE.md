...
- A change to `plugins/wiki-ingest/skills/apply/scripts/` needs `plugins/wiki-ingest/skills/apply/tests/run.sh` to pass. `apply.sh` changes files without a dry-run, because typing `/wiki-ingest:apply` is the approval.
- Only wiki-lint, wiki-query and the `wiki-ingest:apply` skill folder have `scripts/` and `tests/`. Other plugins get them only when a real need shows up.
