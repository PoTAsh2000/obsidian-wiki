...
- A change to `wiki-vault` `overwrite.sh` needs `plugins/wiki-vault/tests/overwrite/run.sh` to pass.
- Only wiki-lint, wiki-query and wiki-vault `overwrite` have `scripts/` and `tests/`. Other plugins get them only when a real need shows up.
