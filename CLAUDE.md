...
- A change to a `wiki-ingest:ingest` script needs `plugins/wiki-ingest/tests/ingest/run.sh` to pass.
- Only wiki-lint, wiki-query and wiki-ingest:ingest (`skills/ingest/scripts/`, `tests/ingest/`) have scripts and tests. Other plugins get them only when a real need shows up.
